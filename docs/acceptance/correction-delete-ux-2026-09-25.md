...
- **385 张历史卡片无 `content_time`**：需执行 `rebuild-index` 管理命令回填（将重新调用 embedding API，产生费用与约数分钟重建窗口）——待用户拍板执行时机
- 时间过滤的日期边界语义已定为 UTC（卡片存储 UTC）；未做用户本地时区换算（V1 无用户时区上下文）
- SIMTEST 遗留 O-01（写入→可检索 ~7s 延迟，index_state 提示已缓解体验）保持现状
